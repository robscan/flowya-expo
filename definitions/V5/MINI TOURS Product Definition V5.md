# MINI TOURS
## Product Definition V5 · Antecedente Oficial para Desarrollo
...
Mini Tours existe para ayudar a las personas a notar lugares, no a completar listas.
No se trata de ver más.
Se trata de elegir mejor.

Mini Tours no optimiza el viaje.
Lo afina.
...
En Mini Tours no se empieza creando contenido.
...
- **Flow**: el estado activo de movimiento.
- **Map**: exploración libre y planeación.
- **Gems**: lugares y paths que brillan ahora.
- **Saved**: memoria personal.
- **Not my vibe**: señal de afinidad negativa (no juicio).
...
Incluye Spots destacados, Spots recientes y Paths sugeridos.
...
Incluye Spots y Paths guardados, así como historial de navegación.
...
Search permite encontrar Spots y Paths.
La búsqueda es contextual y adaptativa.
...
#### ANTICIPACIÓN
Se activa antes de llegar al Spot.
Prepara la mirada.
Duración corta.

#### PRESENCIA
Se activa al llegar.
Ancla al usuario en el momento.
Duración breve y evocadora.

#### TRANSICIÓN
Se activa al salir.
Cierra sin romper el Flow.

#### CONTEXTO DE PATH
Aparece entre Spots.
Da coherencia emocional al recorrido.
Nunca constante.
...
### NARRACIÓN Y AFINIDAD

Las señales del usuario influyen en la narración.
- Likes refuerzan cierto tono.
- Not my vibe reduce narraciones similares.

La narración se adapta sin fricción.

### IDIOMA

La narración es multi-idioma por diseño.
Puede traducirse o localizarse.
Debe sentirse nativa, no literal.

---

## FASES DE DESARROLLO

Mini Tours se desarrolla en fases claramente diferenciadas.
...
Cuando Mini Tours está listo para usuarios reales, se agregan capas de experiencia.
...
La interfaz nunca debe competir con el lugar.
Debe sentirse calma, presente y casi invisible.

---

## PLAN DE TRABAJO / ACTIVIDADES

### Reestructuración de Pantallas (V5.1)

**Gems Screen:**
- [x] Actualizar definición V5 con nueva estructura
- [x] Reestructurar gems.tsx para usar sliders horizontales
- [x] Implementar header scrollable
- [x] Convertir secciones de spots a sliders horizontales
- [x] Mantener lista vertical para paths sugeridos

**Saved Screen:**
- [x] Actualizar definición V5 con nueva estructura
- [x] Cambiar tabs a "Saved" e "History"
- [x] Implementar header scrollable
- [x] Tab "Saved": Sliders horizontales de spots y paths guardados
- [x] Tab "History": Lista vertical de items visitados sin guardar
- [x] Crear función renderPathSlider para paths en slider horizontal

### Mover Liked Spots a Profile (V5.2)

**SavedContext:**
- [ ] Agregar campo `likedSpotsFromPlayer` para diferenciar likes del player
- [ ] Agregar función `toggleLikeSpotFromPlayer(spotId: string)`
- [ ] Agregar función `isSpotLikedFromPlayer(spotId: string)`
- [ ] Exportar `likedSpotsFromPlayer` en el contexto

**Player Components:**
- [ ] Agregar botón de like en `FlowFullPlayer` para spot actual
- [ ] Agregar botón de like en `FlowScreen` para spot actual (si aplica)
- [ ] Usar `toggleLikeSpotFromPlayer` cuando se hace like desde el player

**Profile Screen:**
- [ ] Agregar sección "Liked Spots" con slider horizontal
- [ ] Implementar header scrollable (igual que otras pantallas)
- [ ] Mostrar solo spots de `likedSpotsFromPlayer`
- [ ] Usar misma estructura visual que Saved (sliders horizontales)

**Definición V5:**
- [x] Actualizar sección PROFILE para incluir "Liked Spots"

### Convertir Spot y Path Detail a Secciones Completas (V5.3)

**Definición V5:**
- [x] Actualizar sección "ARQUITECTURA GENERAL DE PANTALLAS"
- [x] Agregar sección "SPOT DETAIL" con layout completo
- [x] Agregar sección "PATH DETAIL" con layout completo
- [x] Actualizar referencias a SpotDetailSheet por Spot Detail

**Conversión de Spot Detail:**
- [x] Crear `app/spot-detail.tsx` como pantalla completa
- [x] Migrar contenido de `SpotDetailSheet.tsx` a la nueva pantalla
- [x] Implementar header con imagen grande y overlay de controles
- [x] Implementar sección de contenido con esquinas redondeadas
- [x] Implementar secciones scrollables ("Why it matters", "Cultural context", "How to visit", "Plan info")
- [x] Implementar botón de acción ("Start from here")
- [x] Agregar ruta en `app/_layout.tsx`
- [x] Actualizar todos los lugares que abren `SpotDetailSheet` para navegar a `/spot-detail`
- [x] Agregar sección "Location" con mapa justo antes de "How to visit"
- [ ] Ajustar iconos del header a envolvente circular según convenciones (dark mode: fondo negro/icono blanco, estado activo: color tint)
- [ ] Implementar menú de opciones (tres puntos) con: "Suggest an edit", "Reportar", "Este lugar ya no existe"

**Creación de Path Detail:**
- [ ] Crear `app/path-detail.tsx` como pantalla completa
- [ ] Implementar header con back, like, share
- [ ] Implementar tags y descripción
- [ ] Implementar sección de métricas (duración, distancia, dificultad, momento)
- [ ] Implementar botón "Start Playlist" con nota de optimización
- [ ] Implementar sección "Playlist" con lista de spots
- [ ] Implementar funcionalidad de reordenamiento
- [ ] Agregar ruta en `app/_layout.tsx`
- [ ] Actualizar todos los lugares que deberían abrir Path Detail para navegar a `/path-detail`

**Limpieza:**
- [ ] Eliminar `components/SpotDetailSheet.tsx` (o mantener como referencia temporal)
- [ ] Verificar que todas las navegaciones funcionen correctamente
- [ ] Ajustar estilos y espaciados según diseño
- [ ] Probar en diferentes tamaños de pantalla

**Nota importante:**
- Path Detail aún no ha sido creado. Esta es la siguiente tarea pendiente (Fase 3 del plan original).
- Path Detail debe ser creado como `app/path-detail.tsx` siguiendo el mismo patrón que Spot Detail.
